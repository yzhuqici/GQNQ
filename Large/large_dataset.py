...
class LargeStateMeasurementResultData(Dataset):
    def __init__(self,num_observables=27,num_states=100,num_qubits=20):
        ...
        for j in range(0,num_states):
            tmp = np.load('probs/' + str(num_qubits) + 'qubits_'+str(j)+'.npy')
            tmp = tmp.reshape(-1, 4)
...
class LargeRandomState3qubitMeasurementResultData(Dataset):
    def __init__(self,num_observables=216,num_states=50,num_qubits=20):
        observables = []
        for i in range(0,num_observables):
            tmp = np.load('3qubit/float_observable3'+str(i)+'.npy')
            observables.append(np.array(tmp))

        index_observables = []
        combination_list = np.load('mso/' + str(num_qubits)+'qubit_sequence_list_3qubit.npy')
        for j in range(0, len(combination_list)):
            for i in range(0,num_observables):
                tmp = np.concatenate((observables[i],combination_list[j]))
                index_observables.append(tmp)
        ...
        for i in range(0,41):
            if i >=15 and i <25:
                continue
            for j in range(0,num_states):
                tmp = np.load('ising_rand/rand_3qubit_prob' + str(num_qubits) + 'qubits_' + str(round(ratio_space[i],1)) + '_' + str(j)+'.npy')
                tmp = tmp.reshape(-1, 8)
                values.append(np.array(tmp,dtype=np.float32))
        self.values = np.array(values)
...
class TestLargeRandomState3qubitMeasurementResultData(Dataset):
    def __init__(self,num_observables=27,num_states=100,num_qubits=20):
        observables = []
        for i in range(0,num_observables):
            tmp = np.load('3qubit/float_observable3'+str(i)+'.npy')
        ...
        for i in range(0,41):
            ...
            #     continue
            for j in range(0,num_states):
                tmp = np.load('ising_rand/test_3qubit_rand_prob' + str(num_qubits) + 'qubits_' + str(round(ratio_space[i],1)) + '_' + str(j)+'.npy')
                tmp = tmp.reshape(-1, 8)
                values.append(np.array(tmp,dtype=np.float32))
        self.values = np.array(values)
